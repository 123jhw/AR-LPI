...
def read_pair_file(fpath):
    ...
    for line in flines:
        ...
        p1, p2 = X.split(" ")
        if isPrint : 
            print("P1: {} / P2: {} / Label: {}".format(p2,p1,label))
        
        pairs.append((p2,p1,label))
    return pairs
...
def preprocess_feature(x, y, npz_path):
    ...
    feature_x = []
    r_mer = 3
    r_CTF = improvedCTF(letters=["A","C","G","T"],length=r_mer)
        ...
        for mer in range(1,r_mer+1):
            for i in range(0,len(rseq)-mer):
                ...
                try:
                    ...
                    continue
                #print(pattern)
        Rw = np.array([rseq.count("A")/len(rseq),rseq.count("C")/len(rseq),rseq.count("G")/len(rseq),rseq.count("T")/len(rseq)])
        #print(Rw)
        r_E = {}
        for i in range(0,4):
            Er1 = Rw[i]
